...
fallow %>% 
  ggplot(aes(x=striga, y=yield))+
  geom_point()

fallow %>%
  filter(striga>0)%>%
  ggplot(aes(x=striga, y=yield, color=treat, group=treat))+
  geom_point()+
