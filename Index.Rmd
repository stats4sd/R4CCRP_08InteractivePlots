...
LongData <- MergedData %>%
  pivot_longer(cols=Control:Treatment, names_to="Parcel_type",values_to="Yield")
...
( LongData %>%
  ggplot(aes(x=Parcel_type, y=Yield, color=Accept, group=Farmer, gender=Gender, latitude=Latitude, longitude=Longitude))+
    geom_line(lwd=1.2)+
      labs(title="Yield by parcel type and willingness\nto participate in new experiment",
           color="Accept?") ) %>%
          ggplotly()
...
Let's say we are working with the LongData of the datasets we have used a lot in this course, the `Pulse` survey data. 
...
LongData %>%
  ggplot(aes(x=Parcel_type, y=Yield, color=Accept, group=Farmer))+
    geom_line(lwd=1.2)+
      labs(title="Yield by parcel type and willingness\nto participate in new experiment",
           color="Accept?")
...
Notice that if you hover over any of the points on the Control or Treatment vertical you see a pop up with the underlying data values.

An additional nice feature you can use here is to add in extra aesthetics which will only appear in the pop-up box. So for example if you wanted to also see `Gender`, `Latitude` and `Longitude`.
...
This has worked quite well for displaying the points with lots of additional data included in the pop up box.
